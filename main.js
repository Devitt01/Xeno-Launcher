...
function buildInstallerApplyScript({ installerPath, statusPath, handshakePath, marker, exePath, launcherPid }) {
  ...
  return [
    ...
    'if($ok){',
    '  try {',
    '    $prePayload = @{ ok = $true; marker = $marker; mode = "setup"; preStart = $true; time = (Get-Date).ToString("o") } | ConvertTo-Json -Compress',
...
function launchPortableSelfUpdater(downloadedExePath, statusFilePath, marker = '', options = {}) {
  try {
    ...
    const script = [
      ...
      'if($ok){',
      '  try {',
      '    $prePayload = @{ ok = $true; marker = $marker; mode = "portable"; preStart = $true; time = (Get-Date).ToString("o") } | ConvertTo-Json -Compress',
      '    New-Item -ItemType Directory -Path (Split-Path -Parent $status) -Force | Out-Null',
      '    Set-Content -LiteralPath $status -Value $prePayload -Encoding UTF8 -Force',
      '  } catch {}',
      '  Start-Sleep -Milliseconds 250',
...
function buildAsarApplyScript({ src, dst, backup, exe, statusPath, marker, elevated, launcherPid, expectedSha1 }) {
  ...
  return [
    ...
    '  try {',
    '    $payload = @{ stage = [string]$stage; ok = $ok; marker = $marker; elevated = $elevated; expectedSha1 = [string]$expectedSha1; appliedSha1 = [string]$appliedSha1; dst = [string]$dst; error = [string]$errorMessage; restarted = $started; restartError = [string]$startError; time = (Get-Date).ToString("o") } | ConvertTo-Json -Compress',
